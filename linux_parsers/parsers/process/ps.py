...
def parse_ps_aux(command_output: str) -> list[dict[str, str | Any]]:
...
def parse_ps_ax(command_output: str) -> list[dict[str, str | Any]]:
    """Parse `ps -ax` command output."""
    lines = '\n'.join([i.strip() for i in command_output.splitlines() if i.strip()])
    pattern = re.compile("(?P<pid>\d+)\s+(?P<tty>\S+)\s+(?P<stat>\S+)\s+(?P<time>\S+)\s+(?P<command>.+)")
